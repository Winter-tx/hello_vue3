<template>
    ...
    <div class="person">
        ...
        <!-- 1. 测试有setup时, 可以继续写date与method -->
        <h2>测试有setup时, 能否继续写date与method; data: {{ testData }}</h2>
        <button @click="testFun">测试有setup时, 能否继续写date与method</button>

        <!-- 2. data可以调用setup中的数据, 因为setup的声明周期早于data, 但需要使用this关键字获取-->
        <h2>data可以调用setup中的数据; testGetSetupData: {{ testGetSetupData }}</h2>
...
export default {
    ...
    */
    ...
    setup() {
        ...
        // setup的返回值也可以是一个渲染函数, 函数的返回值会直接渲染在页面上, 无需<template>模板调用
        // return function(){
        //     return "fun-return"
        // }1   
        // 或者简化为箭头函数
